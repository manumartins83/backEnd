...
<head>
    ...
    <title>Tickets retour : <?php echo $client['Nom_Client']; ?></title>
    ...
    <!-- affichage formulaire de sélection bons de commande client -->
    <form class="stylePageAfficheTicket" method="post">
        ...
        <div>
            <select class="styleSelectAfficheTicket" name="num_bon_cde">
                <?php
                // requête de récupération Id utilisateur connecté dans base de données table bons de commande
                $sql = "SELECT * FROM t_d_commande WHERE Id_Client=:useid";
                $stmt = $pdo->prepare($sql);
                $stmt->bindValue(':useid', $client['Id_Client']);
                $stmt->execute();

                // tant que données bons de commande client correspondant présent dans base données alors
                while ($numCde = $stmt->fetch()) { ?>
                    <option value="<?= $numCde['Id_Commande'] ?>"><?= $numCde['Num_Commande']  // affichage de la liste des bons client correspondant dans select ?></option>
                <?php } ?>
            </select>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // tant que num bon de commande client correspondant présent dans base données alors
        while ($numCde = $stmt->fetch()) {
        ...
        if ($stmt->rowCount() > 0) {
            // tant que tickets retour correspondant à un bon de commande client dans base de données alors
            while ($ticket = $stmt->fetch()) {
                // affichage de la liste des tickets correspondant au bon commande client
