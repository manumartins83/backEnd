...
$payment_options = $conn->query("SELECT * FROM t_d_type_paiement")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO t_d_commande (Date_Commande, Id_Statut,
    ...
    $stmt = $conn->prepare("INSERT INTO t_d_expedition (Date_Expedition) VALUES (null)");
    $stmt->execute();
    // récupérer l'id de l'expedition créé
    $exp_id = $conn->lastInsertId();



    //insérer les infos d'adresse dans la table t_d_adresse puis, t_d_adressecommande (1 pour livraison, 2 pour factu)
    $stmt = $conn->prepare("INSERT INTO t_d_adresse (Ligne1_Adresse, Ligne2_Adresse, Ligne3_Adresse,
    ...
    $stmt->bindValue(':client', $client['Id_Client']);
    $stmt->execute();
    $deliv_id = $conn->lastInsertId();

    $stmt = $conn->prepare("INSERT INTO t_d_adressecommande
    ...
    foreach ($_SESSION['cart'] as $id => $quantity) {
        ...
        $stmt = $conn->prepare("INSERT INTO t_d_lignecommande
